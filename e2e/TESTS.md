...
| File | What it covers |
| --- | --- |
| [`tests/smoke.spec.ts`](./tests/smoke.spec.ts) | Homepage loads; the header profile photo and completion badge are not clipped; unauthenticated users browse Discover (including **Groups you are visible to** / **Groups visible to you** and the Everyone group filter), the group filter choice survives reload, skipping someone keeps another card in the feed, **Open to meet** shows next to last-seen on opted-in profiles, Chats requires a session, and the notification icon (`/apple-touch-icon.png`) is served as a PNG referenced by `sw.js`. |
...
| [`tests/push.spec.ts`](./tests/push.spec.ts) | With `Notification` stubbed so permission stays `default` (what a dismissed prompt leaves behind), signing in asks for notification permission again on the next session instead of remembering that it already asked. |
